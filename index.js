...
webpush.setVapidDetails(
  "mailto: nallapaneni.sreehari@gmail.com",
  keys.publicVapidKey,
...
app.post("/subscribe", async (req, res) => {
  ...
  var subscriber = {
    ...
    browserInfo: {
      browser: req.body?.clientInfo?.browser,
      browserVersion: req.body?.clientInfo?.browserVersion,
    },
    osInfo: {
      os: req.body?.clientInfo?.os,
      osVersion: req.body?.clientInfo?.osVersion,
    },
    ipAddress: ip,
    location: location,
  ...
  let subscriptionExists = await subscriberModel.find({
...
app.post("/updateViewsClicks", async (req, res) => {
  ...
  if (req.body?.views) {
    await notificationModel.findOneAndUpdate(
      { _id: ObjectId(req?.body?.views?.id) },
      { $inc: { views: 1 } }
    );
  } else if (req.body?.clicks) {
    await notificationModel.findOneAndUpdate(
      { _id: ObjectId(req?.body?.clicks?.id) },
      { $inc: { clicks: 1 } }
    );
  }
});
